...
export const adminReports = {
  getAll: (params?: {
    ...
    extendedApiClient.request(
      `/admin-reports${params ? `?${new URLSearchParams(params as any).toString()}` : ''}`
    ),
  getById: (id: string) => extendedApiClient.request(`/admin-reports/${id}`),
  create: (data: {
    title: string;
    description?: string;
    targetUserType: string;
    ...
    extendedApiClient.request('/admin-reports', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  update: (id: string, data: any) =>
    extendedApiClient.request(`/admin-reports/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
    extendedApiClient.request(`/admin-reports/${id}`, { method: 'DELETE' }),

  // Form Configuration
  getFormConfig: (id: string) =>
    extendedApiClient.request(`/admin-reports/${id}/form-config`),
  ...
  publishFormConfig: (id: string, data: { isPublished: boolean }) =>
    extendedApiClient.request(`/admin-reports/${id}/form-config/publish`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  ...
  submitForm: (id: string, submissionId: string, data?: { formData?: any }) =>
    extendedApiClient.request(
      `/admin-reports/${id}/submissions/${submissionId}/submit`,
      {
        method: 'PATCH',
  ...
  updateSubmission: (id: string, submissionId: string, data: { formData: any }) =>
    extendedApiClient.request(`/admin-reports/${id}/submissions/${submissionId}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
