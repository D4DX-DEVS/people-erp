  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (recentData?.data?.applications) setRecentApplications(recentData.data.applications);
      } catch (err: any) {
          ...
          {recentApplications.length === 0 ? (
            ...
            <div className="divide-y">
              {recentApplications.map((app: any, idx: number) => (
                <div key={app.id ?? idx} className="flex items-center justify-between py-3 cursor-pointer hover:bg-muted/30 rounded px-2 -mx-2 transition-colors" onClick={() => navigate(app._id ? `/applications/all?application=${app._id}` : '/applications/all')}>
                  <div>
