...
export default function AdminReportFormBuilder() {
  ...
  const [formTitle, setFormTitle] = useState("Report Form");
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res.data?.hasConfiguration) {
          ...
          // New form — pre-fill title from report
          if (reportTitle) {
  ...
  // ── Field / page helpers (mirrors FormBuilder.tsx) ─────────────────────────
  ...
  const saveFormConfiguration = async () => {
    if (!reportId) {
      toast({ title: "No report context. Navigate from the Reports page.", variant: "destructive" });
      return;
    }
    setSaving(true);
    try {
      await adminReports.updateFormConfig(reportId, {
        title: formTitle,
  ...
  const publishForm = async () => {
    ...
    setSaving(true);
    try {
      await adminReports.publishFormConfig(reportId, { isPublished: !isPublished });
  ...
  const exportFormJSON = () => {
    ...
    a.href = url;
    a.download = "report-form-config.json";
    a.click();
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Only super admins can configure report forms.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => navigate("/admin-reports")} className="flex items-center gap-2">
            <ArrowLeft className="h-4 w-4" />
            <span className="hidden sm:inline">Back to Reports</span>
          </Button>
          <div>
            <h1 className="text-lg font-bold">Report Form Builder</h1>
            {reportTitle && (
      ...
      {!reportId && (
        <Alert>
          <AlertDescription>
            No report context. Navigate here from the <strong>Reports</strong> page using "Configure Form".
          </AlertDescription>
        </Alert>
      )}
