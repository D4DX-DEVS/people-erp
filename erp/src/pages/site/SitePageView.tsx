...
import {
  Users, Heart, Target, Eye, Home as HomeIcon, Briefcase, Stethoscope, GraduationCap,
  HandHeart, Sparkles, Star, Globe, Phone, BookOpen, ChevronRight, Play, FileText, Download,
} from "lucide-react";
...
const ICONS: Record<string, any> = {
  users: Users, heart: Heart, target: Target, eye: Eye,
  home: HomeIcon, housing: HomeIcon, briefcase: Briefcase, livelihood: Briefcase,
  health: Stethoscope, healthcare: Stethoscope, education: GraduationCap, graduation: GraduationCap,
  hands: HandHeart, sparkles: Sparkles, star: Star, globe: Globe, phone: Phone, book: BookOpen,
};
...
function SectionBlock({ section }: { section: PageSection }) {
  ...
  switch (section.type) {
    ...
    case "cards": {
      ...
      body = (
        <div className={`grid gap-6 ${colsClass(section.columns)}`}>
          {items.map((item, idx) => (
            <CardTile key={item._id || idx} item={item} onLink={goLink} />
          ))}
        </div>
      );
      break;
          ...
          {items.map((item, idx) => {
            ...
            return (
              <div key={item._id || idx} className="text-center">
                ...
                <div className="text-3xl font-extrabold text-foreground md:text-4xl">{item.value}</div>
    ...
    case "timeline": {
      if (!items.length) return null;
      body = (
        <div className="relative mx-auto max-w-2xl border-l-2 border-primary/30 pl-8">
          {items.map((item, idx) => (
            <div key={item._id || idx} className="relative pb-10 last:pb-0">
              ...
              {item.value && <div className="text-sm font-bold text-primary">{item.value}</div>}
  ...
  return (
    ...
    <section className={`py-14 md:py-20 [overflow-wrap:anywhere] ${bgClass}`}>
      <div className="container mx-auto px-4">
...
function CardTile({ item, onLink }: { item: SectionItem; onLink: (link?: string) => void }) {
  const Icon = iconFor(item.icon);
          ...
          {items.map((p) => (
            <Card key={p._id} className="group overflow-hidden border-border/60 transition-shadow hover:shadow-xl">
              <div className="relative h-40 overflow-hidden">
                <img src={categoryImage(p.category)} alt={p.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
